...
class MulToLayerNormPat(CanonicalizeBase):
    ...
    def rewrite(self, g:Graph, op: MulOp):
        ...
        g.add_node(new_op)
        for _op_name in self.delops:
            _op = g.node_table[_op_name]
            _parents = [g.node_table[i] for i in _op.parents]
            g.delete_node(_op, _parents)
