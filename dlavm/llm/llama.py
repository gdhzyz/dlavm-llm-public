...
def llama_block_hbm(data, pos_weight, silu_weight, token, index, path_prefix="BLOCK_write_data", ir=False):
...
def llama_expr_hbm(device, path_prefix, debug, ir=False):
    ...
    for n in range(block_size):
        data = llama_block_hbm(data, pos_weight, silu_weight, token, n, path_prefix=path_prefix, ir=ir)
...
def llama_block(data, pos_weight, silu_weight, token, index, path_prefix="BLOCK_write_data"):
    prefix = "BLOCK%02d_" % index
    data_path = path_prefix + "/BLOCK%02d/" % index
    ln_k_bias = adr.hbm.const_ddr(prefix + "ln_k_bias", data_path + "LN_DDR_bin/LN0_wt_in_DDR.bin", [4096*2])
    q_weight = adr.hbm.const_hbm(prefix + "q_weight", data_path + "MVM_BN_write_to_HBM_bin/MVMBN0_q_HBM_DDR_%02d.bin", [4096, 128*32])
...
def llama_expr(device, path_prefix, debug):
    block_size = 1 if debug else 32
    ...
    last_token = ne.Var("last_token", device.MAX_TOKEN)
    for n in range(block_size):
        data = llama_block(data, pos_weight, silu_weight, token, n, path_prefix=path_prefix)
    ln_k_bias = adr.hbm.const_ddr("Final_LN_k_bias", path_prefix + "/OutLayer/LN_DDR_bin/LN_wt_in_DDR.bin", [4096*2])
