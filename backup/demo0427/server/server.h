...
typedef void (*Module)(HANDLE, int, int);

uint16_t embeddings [65024*4096];

/*
 * Start FPGA Simulation
 */
int index = 0;
uint16_t results[4] = {36474, 54591, 243, 2};

void test(HANDLE device, int token, int kvcache) {
    printf("model: %d, %d\n", token, kvcache);
}
/*
 * End FPGA Simulation
...
typedef uint16_t (*exe_t)(RPCClient*, std::string);
  public:
    ...
    static uint16_t run_model(RPCClient* client, std::string attrs) {
        ...
        mod(client->accel, token, kvcache);
        return get_output(client->c2hx, client->data_out);
    }
